...
describe("GET /confirm", () => {
    ...
    it("should not activate user if request invalid (no secret)", (done) => {
        request(app)
            .get("/confirm?email=something-invalid")
            .expect(400)
            .end(done);
    });
});
    ...
    it("should create userDetails for seller if authenticated", (done) => {
        ...
        request(app)
            ...
            .end((err) => {
                ...
                UserDetails.findOne({ _creator: users[0]._id }).then((details) => {
    ...
    it("should create userDetails for buyer if authenticated", (done) => {
        ...
        request(app)
            ...
            .end((err) => {
                ...
                UserDetails.findOne({ _creator: users[1]._id }).then((details) => {
                    expect(details.name).toBe(body.name);
                    expect(details.address).toBe(body.address);
                    done();
                }).catch(e => done(e));
    ...
    it("should create userDetails for verifier if authenticated", (done) => {
        ...
        request(app)
            ...
            .end((err) => {
                ...
                UserDetails.findOne({ _creator: users[2]._id }).then((details) => {
                    expect(details.name).toBe(body.name);
                    expect(details.address).toBe(body.address);
                    done();
                }).catch(e => done(e));
            ...
            request(app)
                ...
                .end((err) => {
                    ...
                    if (key === "name" || key === "address") {
                        UserDetails.findOne({ _creator: user._id }).then((details) => {
